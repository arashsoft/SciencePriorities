...
angular.module('sciencePriorities2App')
    .directive('keywordExt', function () {
        ...
        keywordExtDirective.link = function (scope, element, attr) {
                ...
                if(scope.param1.model === '') {
                    ...
                    if(scope.param2.selectedAwardID == -1) {
                        ...
                        $.get('/jsonrequest2/awardAnalysisSelect/' + JSON.stringify(analysisObject) , function(analyzedAward){
                            ...
                            d3.selectAll('.authorLegendName').remove().transition().ease('elastic');*/
            ...
            function drawGraph(analyticsAward, analyticsSetting, publicationsSelected) {
                // constants and variables that determine the scales
                var publicationsMinRange = analyticsSetting.publicationMinRange,
                    publicationsMaxRange = analyticsSetting.publicationMaxRange;

                //for award
                var awardKeywords = analyticsAward._awardKeywords;
                var awardTopics = analyticsAward._ldaResult;
                var awardKeywordsNumber = _.size(analyticsAward._awardKeywords);
                var awardBeginDate = parseInt(analyticsAward._awardBeginDate.slice(0,4));
                var awardEndDate = parseInt(analyticsAward._awardEndDate.slice(0,4));

                //for publications
                var publicationsNumber = _.size(analyticsAward._relatedPublicationsList);
                var yearValues = _.uniq(_.pluck(analyticsAward._relatedPublicationsList, '_year')).sort();
                var analyticsAwardModified = orderByYear(analyticsAward);
                var maxPublicationNumberPerYear = _.size(analyticsAwardModified._relatedPublicationsList[yearValues[0]]);
                var minPublicationNumberPerYear = _.size(analyticsAwardModified._relatedPublicationsList[yearValues[0]]);
                yearValues.forEach(function(year) {
                ...
                /*var awardKeywordTip = d3.tip()
                 .attr('class', 'd3-tip')
                 .offset([-10, 0])
                 .html(function(d) {
                 return "<strong>Keyword:</strong> <span style='color:black'>" + d + "</span>";
                 });

                 analysisSVGContainer.call(awardKeywordTip);*/

                var awardTopicTip = d3.tip()
                    .attr('class', 'd3-tip')
                    .offset([-10, 0])
                    .html(function(d) {
                        return "<strong>Topic Terms:</strong> <span style='color:black'>" + d + "</span>";
                    });

                analysisSVGContainer.call(awardTopicTip);
                    ...
                    .html(function(d) {
                        if(analyticsSetting.model == 'Specific') {
                            var tip = "<strong>Title:</strong> <span style='color:black'>" + d._title + "</span>" +
                                ...
                                "<strong>Authors: </strong><span style='color:black'>" + d._authorsArray + "</span>" +
                                "<br>" +
                                "<strong>Confidence: </strong><span style='color:black'>" + d._radius + "</span>";
                ...
                //remove the message for selecting an award
                analysisSVGContainer.selectAll('.message').remove();
                ...
                if(analyticsSetting.model == 'Specific') {
                    ...
                    awardTopics.forEach(function(awardTopic, index) {
                        ...
                        var awardTopicsDivs = awardTopicsDivsGroup.selectAll('text.awardKeywordText')
                            ...
                            .style('stroke', function(d,i) {
                                return awardKeywordColors(base + i);
                            })
                            .each(function(d,i) {
                                var award_topic_info = new Object();
                                award_topic_info.id = 'award-keyword-'+i;
                                award_topic_info.keyword = d.term;
                                award_topic_info.color = awardKeywordColors(i);
                                award_topic_info.x = 0;
                                award_topic_info.y = (base+ 2*padding/5) + i*2*padding/5;
                                ...
                                /* awardTopicsDivsGroup.append('rect')
                                 .attr('class', 'topicTermProbability')
                                 .attr('id', function(d,i) { return 'award-topic-'+index+'-term'+i+'-probability';})
                                 .attr('x', 0)
                                 .attr('y', function(d,i) {
                                 return (base+ 2*padding/5) + i*2*padding/5;
                                 }).
                                 attr('width', function(d,i) {
                                 return d.probability*padding;
                                 })
                        ...
                        //base = (index+1)*(termNumber+1)*2*padding/5;
                    });
                }
                else {
                    analysisSVGContainer.append('text')
                ...
                analysisSVGContainer.append('text')
                    ...
                    .attr('transform', 'translate(' + (chartWidth-4*padding) + ',' + padding/5 + ')')
                    .attr('class', 'awardInfo')
                    .attr('id', 'selectedPublications')
                    .style('text-decoration', 'underline')
                    .style('font-weight', 'bold');
                ...
                if(analyticsAwardModified._error) {
                    ...
                    var graphRootContainer = analysisSVGContainer
                        .append('g')
                        .attr('id', 'graph-root')
                        .attr('transform', 'translate(' + 2.5*padding + ',' + 4*padding/5 + ')');
                    ...
                    var publicationYearsAxisMin = Math.min(publicationsMinRange, awardBeginDate);
                    var publicationYearsAxisMax = Math.max(publicationsMaxRange, awardEndDate);

                    var publicationYearsMargin = 1.20; //the less this margin be the more the distance between publication years

                    var publicationsYearsScale = d3.scale.linear()
                        .domain([publicationYearsAxisMin-1, publicationYearsAxisMax])
                        .range([(chartHeight - padding)/publicationYearsMargin, padding]);

                    var publicationsYearsAxis = d3.svg.axis()
                        .scale(publicationsYearsScale)
                        .orient('left')
                        .ticks(publicationsMaxRange - publicationsMinRange + 1)
                        .tickFormat(d3.format('1'));

                    graphRootContainer.append('g')
                        .attr('class', 'yearAxis')
                        .attr('transform', 'translate(' + padding*0.95 + ',' + 0 + ')')
                        .call(publicationsYearsAxis);

                    /*///////////////////////////////////////////////////////////////////////////////*/

                    var publicationsScaleRange = chartWidth-8*padding;
                    var publicationsScale = d3.scale.linear()
                        .domain([0, 15])
                        .range([0, publicationsScaleRange]);

                    var publicationsAxis = d3.svg.axis()
                        .scale(publicationsScale)
                        .orient('top')
                        .ticks(15)
                        .tickFormat(d3.format('1'));
                    ...
                    graphRootContainer.append('g')
                        .attr('class', 'yearAxis')
                        .attr('transform', 'translate(' + padding + ',' + padding*0.90 + ')')
                        .call(publicationsAxis);

                    /*///////////////////////////////////////////////////////////////////////////////*/

                    /*var awardKeywordsMargin = 4; //the less this margin be the more the distance between award keywords
                     var awardKeywordsScale = d3.scale.linear()
                     .domain([0, awardKeywordsNumber-1])
                     .range([publicationsYearsScale(awardEndDate), publicationsYearsScale(awardBeginDate)-25]);

                     var awardKeywordsAxis = d3.svg.axis()
                     .scale(awardKeywordsScale)
                     .orient('right')
                     .ticks(awardKeywordsNumber-1);

                     graphRootContainer.append('g')
                     .attr('class', 'keywordAxis')
                     .call(awardKeywordsAxis);*/

                    /*///////////////////////////////////////////////////////////////////////////////*/

                    yearValues.forEach(function(year) {
                        analyticsAward._relatedPublicationsList[year].forEach(function(publication) {
                            publication._clicked = false;
                        });
                    });

                    var selectedPublications = new Array();
                    var publicationsKeywordsInfo = new Array();
                    var publicationKeywordRectWidth = 60;
                    var publicationKeywordRectHeight = 15;
                    var publicationConfidenceRectHeight = 5;

                    yearValues.forEach(function(year) {
                        var year_group = graphRootContainer.append('g')
                            .attr('id', year + '-publications')
                            .attr('transform', 'translate(' + padding + ',' + publicationsYearsScale(year) + ')');
                        publicationKeywordRectHeight = (publicationsYearsScale(year) - publicationsYearsScale(year+1));

                        year_group.selectAll('rect.publicationKeywordDiv')
                            .data(analyticsAward._relatedPublicationsList[year].slice(0,15))
                            .enter()
                            .append('rect')
                            .on('mouseover', publicationTip.show)
                            .on('mouseout', publicationTip.hide)
                            .on('click', function(d,i) {
                                if(!d._clicked) {
                                    ...
                                    d3.select(this).style('stroke-dasharray', '0,0');
                                    selectedPublications.splice(selectedPublications.indexOf(d), 1);
                                    if(_.size(selectedPublications) < 1) {
                                        graphRootContainer.selectAll('.metadataDiv').remove();
                                    }
                                }
                            })
                            .attr('class', 'publicationKeywordDiv')
                            .attr('id', function(d,i) {
                                return year + '-publication-keyword-'+i;
                            })
                            .attr('x', function(d, i) { return publicationsScale(i) + 4})
                            .attr('y', 0)
                            .attr('width', publicationKeywordRectWidth-8)
                            .attr('height', publicationKeywordRectHeight-15)
                            .each(function(d,i) {
                                var publication_keyword_info = new Object();
                                publication_keyword_info.id = year + '-publication-keyword-'+i;
                                publication_keyword_info.year = year;
                                publication_keyword_info.keywords = d._keywords;
                                publication_keyword_info.authorKeywords = d._authorKeywords;
                                publication_keyword_info.indexKeywords = d._indexKeywords;
                                publication_keyword_info.title = d._title;
                                publication_keyword_info.x = publicationsScale(i) + padding + 4;
                                publication_keyword_info.y = publicationsYearsScale(year);

                                publicationsKeywordsInfo.push(publication_keyword_info);
                            });


                        // TODO choose between the hue method or the progress bar

                        // THE HUE METHOD
                        /*var confidenceHue = d3.scale.linear()
                         .domain([0, 1])
                         .range(["#FFF", "#000"])
                         .interpolate(d3.interpolateHcl);

                         year_group.selectAll('rect.publicationConfidenceDiv')
                         .data(analyticsAward._relatedPublicationsList[year].slice(0,15))
                         .enter()
                         .append('rect')
                         .attr('class', 'publicationConfidenceDiv')
                         .attr('id', function(d,i) {
                         return year + '-publication-confidence-'+i;
                         })
                         .attr('x', function(d, i) { return publicationsScale(i) + 4})
                         .attr('y', publicationKeywordRectHeight-13)
                         .attr('width', publicationKeywordRectWidth-8)
                         .attr('height', 5)
                         .style('fill', function(d, i) {
                         return confidenceHue(d._radius);
                         });*/
                        ...
                        year_group.selectAll('rect.publicationConfidenceDiv')
                            ...
                            .attr('class', 'publicationConfidenceDiv')
                            .attr('id', function(d,i) {
                                return year + '-publication-confidence-'+i+'-bar';
                            })
                            .attr('x', function(d, i) { return publicationsScale(i) + 4})
                            .attr('y', publicationKeywordRectHeight-13)
                            .attr('width', publicationKeywordRectWidth-8)
                        ...
                        year_group.selectAll('rect.progressBar')
                            .data(analyticsAward._relatedPublicationsList[year].slice(0,15))
                            .enter()
                            .append('rect')
                            .attr('class', 'progressBar')
                            .attr('id', function(d,i) {
                                return year + '-publication-confidence-'+i+'-progress';
                            })
                            .attr('x', function(d, i) { return publicationsScale(i) + 4})
                            .attr('y', publicationKeywordRectHeight-13)
                            .attr('width', function(d,i) {
                                return (publicationKeywordRectWidth-8)* d._radius;
                            })
                            .attr('height', 5);
                    ...
                    /*///////////////////////////////////////////////////////////////////////////////*/
                    /*var keywordTip = d3.tip()
                     .attr('class', 'd3-tip')
                     .offset([-25, 0])
                     .html(function(d) {
                     var tip = "<strong>Term:</strong> <span style='color:black'>" + d.keyword + "</span>";

                     return tip;
                     });

                     analysisSVGContainer.call(keywordTip);*/

                    if(analyticsSetting.model == 'Specific') {
                        publicationsKeywordsInfo.forEach(function(publication) {
                            awardTopicsInfo.forEach(function(awardTerm) {
                                var flag = false;
                                if(publication.keywords.indexOf(awardTerm.keyword) > 0) { flag = true; }
                                if(publication.indexKeywords.contains(awardTerm.keyword)) { flag = true; }
                                if(publication.authorKeywords.contains(awardTerm.keyword)) { flag = true; }
                                if(publication.title.contains(awardTerm.keyword)) { flag = true; }

                                if(flag) {
                                    graphRootContainer.append('rect')
                                        /*.on('mouseover', keywordTip.show)
                                         .on('mouseout', keywordTip.hide)*/
                                        .attr('class', 'correlatedkeywords')
                                        .attr('x', function() {
                                            return publication.x + ((publicationKeywordRectWidth-8)/ _.size(awardTopicsInfo))*awardTopicsInfo.indexOf(awardTerm);
                                        })
                                        .attr('y', publication.y)
                                        .attr('width', ((publicationKeywordRectWidth-8)/ _.size(awardTopicsInfo)))
                                        .attr('height', (publicationsYearsScale(publication.year) - publicationsYearsScale(publication.year+1))-15)
                                        .style('fill', awardTerm.color)
                                        .style('stroke', 'gray')
                                        .transition()
                                        .delay(5000);
                                }
                        ...
                        publicationsKeywordsInfo.forEach(function(publication) {
                            awardKeywordsInfo.forEach(function(awardKeyword) {
                                var flag = false;
                                if(publication.keywords.indexOf(awardKeyword.keyword) > 0) { flag = true; }
                                if(publication.indexKeywords.contains(awardKeyword.keyword)) { flag = true; }
                                if(publication.authorKeywords.contains(awardKeyword.keyword)) { flag = true; }
                                if(publication.title.contains(awardKeyword.keyword)) { flag = true; }
                                ...
                                if(flag) {
                                    graphRootContainer.append('rect')
                                        ...
                                        .attr('class', 'correlatedkeywords')
                                        .attr('x', function() {
                                            return publication.x + ((publicationKeywordRectWidth-8)/awardKeywordsNumber)*awardKeywords.indexOf(awardKeyword.keyword);
                                        })
                                        .attr('y', publication.y)
                                        .attr('width', ((publicationKeywordRectWidth-8)/awardKeywordsNumber))
                                        .attr('height', (publicationsYearsScale(publication.year) - publicationsYearsScale(publication.year+1))-15)
                                        .style('fill', awardKeyword.color)
                                        .style('stroke', 'gray')
                                        .transition()
                                        .delay(5000);
                                }
                    ...
                    /*///////////////////////////////////////////////////////////////////////////////*/
                    var metadataContainer = analysisSVGContainer.append('g')
                        .attr('transform', 'translate(' + (chartWidth-4*padding) + ',' + 3*padding/5 + ')')
                        .attr('id', 'metadataContainer');

                    /* $('#metadataContainer div').first().css('display', 'block');
                     var link = $('#metadataContainer a');
                     link.on('click', function(e) {
                     e.preventDefault();
                     var a = $(this).attr('href');
                     $(a).slideDown('fast');
                     $('#metadataContainer div').not(a).slideUp('fast');
                     });*/

                    //$('#metadataContainer').accordion();


                    Object.observe(selectedPublications, function() {
                        d3.selectAll('.metadataDiv').remove().transition().ease('elastic');
                        d3.selectAll('.metadataText').remove().transition().ease('elastic');
                        d3.select('#metadataContainer').selectAll('*').remove();

                        //a table for the selected publications with a different boxes for each
                        selectedPublications.forEach(function(selectedPublication, index) {
                            /* $('#metadataContainer').append($('<h4>').text('test-'+index));
                             $('#metadataContainer').append($('<div>').append($('<p>').text('test2-'+index)));*/


                            var itemGroup = metadataContainer.append('g')
                                .attr('id', 'publication '+index);

                            /*if(index === 0) {
                             itemGroup.append('a')
                             .attr('href', '#publicationAccordion'+index)
                             .attr('class', 'firstAccordion')
                             .html('testtest');
                             }
                             else {
                             itemGroup.append('a')
                            ...
                            itemGroup.append('rect')
                                .attr('x', 0)
                                .attr('y', padding*index)
                                .attr('width', 3.98*padding)
                                .attr('height', padding/2)
                                .attr('class', 'metadataDiv');

                            itemGroup.append('text')
                                .text(selectedPublication._year + ': ' + ((selectedPublication._title.length <= 29) ? (selectedPublication._title) : (selectedPublication._title.slice(0,29) + ' ...')))
                                .attr('class', 'metadataText')
                                .attr('x', 5)
                                .attr('y', index*padding)
                                .attr('dy', 0.25*padding);
                        });

                        if(_.size(selectedPublications) > 0) {
                            d3.select('#publicationAnalysisContainer').remove();

                            var publicationAnalysisContainer = d3.select(element[0])
                                .append('svg')
                                .attr('width', chartWidth)
                                .attr('height', chartHeight)
                            ...
                            var selectedPublicationIDs = _.uniq(_.pluck(selectedPublications, '_publicationID'));

                            $.get('/jsonrequest2/publicationsSelect/' + JSON.stringify(selectedPublicationIDs) , function(result){
                                ...
                                publicationAnalysisContainer.selectAll('rect.selectedPublicationsAnalysis')
                                    ...
                                    .enter()
                                    .append('rect')
                                    .attr('x', function(d,i) { return 2*padding + (padding*i)/2; })
                                    .attr('y', function(d,i) { return 2*padding + (padding*i)/2; })
                                    .attr('width', 50)
                                    .attr('height', 50)
                                    .style('fill', function(d,i) { return awardKeywordColors(i); });


                                drawMatrix(selectedPublicationIDs, result);
                                if(selectedPublicationIDs.length > 1) {
                                    drawHivePlot(publicationAnalysisContainer, result);
                                }
                            });
                        }
                        else {
                            d3.select('#publicationAnalysisContainer').remove().transition().ease('elastic');

                            publicationAnalysisContainer.selectAll('*').remove().transition().ease('elastic');
                        }
            ...
            function drawMatrix(container, matrixData) {}

            function drawHivePlot(container, hiveData) {
                var authorNodeTip = d3.tip()
                    .attr('class', 'd3-tip')
                    .offset([0, 30])
                    .html(function(d) {
                        var tipContainer =
                            "<strong>Author Name: </strong><span style='color:inherit'>" + d.authorFullName + "</span>";

                        return tipContainer;
                    });

                var uniqPublicationIDs = _.uniq(_.pluck(hiveData, 'publicationID'));
                var uniqAuthorIDs = _.uniq(_.pluck(hiveData, 'authorID'));
                //prepare the hive plot
                var hiveWidth = chartWidth/2;
                var hiveHeight = chartHeight/2;
                var hiveInnerRadius = 20;
                var hiveOuterRadius = 300;
                var hiveAngle = d3.scale.ordinal().domain(d3.range(uniqPublicationIDs.length+1)).rangePoints([0, 2 * Math.PI]);
                var hiveRadius = d3.scale.linear().range([hiveInnerRadius, hiveOuterRadius]);
                var hiveColor = d3.scale.category10();
                ...
                var hiveLinks = new Array();
                /*var hiveLinks = [
                    {source: hiveNodes[0], target: hiveNodes[2]},
                    {source: hiveNodes[1], target: hiveNodes[3]},
                    {source: hiveNodes[2], target: hiveNodes[4]},
                    {source: hiveNodes[2], target: hiveNodes[5]},
                    {source: hiveNodes[3], target: hiveNodes[5]},
                    {source: hiveNodes[4], target: hiveNodes[0]},
                    {source: hiveNodes[5], target: hiveNodes[1]}
                ];*/

                var hiveDataGroupedByPublications = _.groupBy(hiveData, function(item) { return item.publicationID; });
                var hiveDataGroupedByAuthors = _.groupBy(hiveData, function(item) { return item.authorID; });
                hiveData.forEach(function(record) {
                    uniqPublicationIDs.forEach(function(publication, index) {
                        if(record.publicationID === publication) {
                            record.publicationIndex = index;
                        }
                        //record.allAuthors = _.filter(hiveData, function(item) { return item.publicationID == record.publicationID; });
                        record.allAuthors = _.pluck(hiveDataGroupedByPublications[record.publicationID], 'authorID');
                    });
                });

                hiveData.forEach(function(record) {
                    record.x = record.publicationIndex;
                    var authorLocationScale = d3.scale.linear().domain([0, _.size(record.allAuthors)-1]).range([0.1, 0.9]).clamp(true);
                    record.y = authorLocationScale(_.indexOf(record.allAuthors, record.authorID));
                });

                for(i=0; i< _.size(hiveData)-1;i++) {
                    for(j=i+1; j< _.size(hiveData);j++) {
                        if(hiveData[i].publicationIndex != hiveData[j].publicationIndex) {
                            /*log(hiveData[i]);
                            log('vs');
                            log(hiveData[j]);
                            log('---------------------------------------------');*/
                            if(hiveData[i].authorID == hiveData[j].authorID) {
                                //insert a link
                                var temp = new Object();
                                temp.source = hiveData[i];
                                temp.target = hiveData[j];
                ...
                var hivePlot = container.append('g')
                    .attr('transform', 'translate(' + (hiveWidth) + ',' + (hiveHeight) + ')')

                hivePlot.selectAll('.hiveAxis')
                    .data(d3.range(uniqPublicationIDs.length))
                    .enter().append('line')
                    .attr('class', 'hiveAxis')
                    .attr('transform', function(d) { return 'rotate(' + degrees(hiveAngle(d)) + ')'; })
                    .attr('x1', hiveRadius.range()[0])
                    .attr('x2', hiveRadius.range()[1]);

                hivePlot.selectAll('.hiveLink')
                    .data(hiveLinks)
                    .enter().append('path')
                    .attr('class', 'hiveLink')
                    .attr('d', d3.hive.link()
                        .angle(function(d) { return hiveAngle(d.x); })
                        .radius(function(d) { return hiveRadius(d.y); }));

                hivePlot.selectAll('.hiveNode')
                    .data(hiveData)
                    .enter()
                    .append('circle')
                    .attr('transform', function(d) { return 'rotate(' + degrees(hiveAngle(d.x)) + ')'; })
                    /*.on('mouseover', authorNodeTip.show)
                    .on('mouseout', authorNodeTip.hide)*/
                    ...
                    .attr('class', 'hiveNode')
                    .attr('cx', function(d) { return hiveRadius(d.y); })
                    .attr('r', 5);
            }
